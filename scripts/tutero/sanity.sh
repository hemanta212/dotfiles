...
# -----------------------------------------------------------------------------
# Cleanup function - flushes and closes log file descriptor
# -----------------------------------------------------------------------------
cleanup() {
	if [[ -n "$LOG_FD" ]]; then
		# Close the file descriptor to force flush
...
strip_ansi() {
	# Remove ANSI escape sequences from stdin or argument
	if [[ -n "${1:-}" ]]; then
...
# Dual output function - writes to stdout and log file if specified
dual_printf() {
	# shellcheck disable=SC2059
	# Output to terminal
	printf "$@"
	if [[ -n "$LOG_FD" ]]; then
		# Strip ANSI and write to file descriptor
		# shellcheck disable=SC2059
		printf "$@" | strip_ansi >&"$LOG_FD"
	fi
}
...
print_multiline_entries() {
	...
	while IFS= read -r entry; do
		...
		path=${path#/}
		dual_printf '\033[0;35m%s:\033[0m\n' "$path"
	...
	if is_json "$line"; then
        ...
        ')
			...
			if [[ "$OUTPUT_MODE" == "blocks" ]]; then
				...
				if [[ "$has_error_stack" == "yes" ]]; then
					dual_printf '\033[1;33mFormatted Stacktrace:\033[0m\n'
					dual_printf '%s\n' "$formatted_stack"
				fi
				if [[ "$has_error_messages" == "yes" ]]; then
					dual_printf '\033[1;33mError Messages:\033[0m\n'
					dual_printf '%s\n' "$formatted_messages"
				fi
			fi
		else
...
main() {
	...
	while (($# > 0)); do
		...
		--log)
			# Set log file path
			if [[ -z "${2:-}" ]]; then
				log_error "--log requires a file path argument"
				exit 1
			fi
			LOG_FILE="$2"
			# Clear/create the log file and open file descriptor 3
			: >"$LOG_FILE"
			exec 3>>"$LOG_FILE"
			LOG_FD=3
			shift 2
    ...
    some_command | ./sanity.sh --log file.log  # Log to file (without colors)

    Options:
    --jq             Use pure JQ processing (faster, less formatting)
    --log <file>     Write output to specified log file (ANSI colors stripped)
    ...
    - Writes to log file without ANSI color codes when --log is used
    ...
    # Stream logs to terminal and save to file without colors
    go run cmd/main.go | ./sanity.sh --log output.log
EOF
