...
** Pulsar
Highlight current line on-demand or after select operations, By Prot.
#+begin_src emacs-lisp
  (use-package pulsar
    :straight t
    :config
...
** Counsel Dash
Dash powered docsets offline lightning fast in eww
#+begin_src emacs-lisp
...
Insert private comments in any code sourced controlled with git
Need to install server first
https://github.com/masukomi/private_comments#building-from-source

sudo apt install chicken-bin

After ./build.sh, pc will fail but ~private_comments~ exe builds in tgz file extract it and move the executable inside the bin of new folder to ~local/bin~
